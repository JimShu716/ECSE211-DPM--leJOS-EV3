/**
 * This class is used to launch the wall following program, which allows an autonomous tribot
 * to follow a wall of arbitrary shape while maintaining a constant distance between itself and the wall.
 * This class is downloaded on the EV3 processor which then executes the program.
 * 
 * The class instantiates constant parameters and instances of constructors such as the Ultrasonic 
 * Poller, the Controllers and the motors. When the tribot finishes its path, the user stops it by pressing
 * any button on the EV3 processor keypad. 
 * 
